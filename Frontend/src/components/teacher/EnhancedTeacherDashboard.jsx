...
const EnhancedTeacherDashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/sessions/teacher/1`); // Fixed teacher ID
      const data = await response.json();
      
      setStats({
        totalSessions: data.length,
        totalStudents: data.reduce((acc, session) => acc + (session.participant_count || 0), 0),
        totalPolls: data.reduce((acc, session) => acc + (session.poll_count || 0), 0)
      });
    } catch (error) {
